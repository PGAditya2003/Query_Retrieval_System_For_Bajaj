...
async def upload_file(file: UploadFile = File(...)):
    ...
    # Save file (compress if >10MB)
    saved_path, final_name = await save_file(file, file_id)

    # Extract text
    text = extract_text(saved_path, final_name)
    if not text:
    ...
    # Save metadata in DB
    save_metadata(file_id, final_name, saved_path)
